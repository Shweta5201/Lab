...
class RRNNCell(tf.contrib.rnn.RNNCell):
  ...
  def __init__(self, num_units, activation=None, reuse=None, 
    name=None, debug=False, isTraining=True):
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with tf.variable_scope(scope or "basic_rnn_cell"):
      ...
      #                 % inputs_shape)
      #input_depth = inputs_i.shape[1].value
      #inp_kernel = tf.get_variable("input_kernel",
      #  [inputs_depth, self.state_size], dtype=tf.float32)
      inp_bias = tf.get_variable("input_bias",
        [self.state_size], dtype = tf.float32)
      #winp = tf.add(tf.matmul(inputs_i,inp_kernel),inp_bias)
      winp = inputs_i
...
#        "Inputs shape for cell")
      ...
      if self.isTraining:
        new_state = tf.add(wh, winp)
      else:
        new_state = state
      output = self._activation(tf.add(new_state,inp_bias))
      #output = self._activation(inputs_i)
...
class CGRUCell(tf.contrib.rnn.RNNCell):
  ...
  def call(self, inputs, state):
    ...
    gate_inputs = math_ops.matmul(
        array_ops.concat([inputs_i, state], 1), self._gate_kernel)
    ...
    candidate = math_ops.matmul(
        array_ops.concat([inputs_i, r_state], 1), self._candidate_kernel)
